...
PARTS_CENTER_SCHEMA_SQL = """
CREATE TABLE IF NOT EXISTS suppliers (
  id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  shop_id INTEGER NOT NULL REFERENCES shop_profile(id),
  name TEXT NOT NULL,
  website TEXT,
  phone TEXT,
  account_number TEXT,
...
CREATE TABLE IF NOT EXISTS parts (
  id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  shop_id INTEGER NOT NULL REFERENCES shop_profile(id),
  repair_id INTEGER REFERENCES repair_records(id),
...
ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS shop_id INTEGER REFERENCES shop_profile(id);
...
ALTER TABLE parts ADD COLUMN IF NOT EXISTS shop_id INTEGER REFERENCES shop_profile(id);
ALTER TABLE parts ADD COLUMN IF NOT EXISTS repair_id INTEGER REFERENCES repair_records(id);
ALTER TABLE parts ADD COLUMN IF NOT EXISTS supplier_id INTEGER REFERENCES suppliers(id);
ALTER TABLE parts ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS part_number TEXT;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS quantity DOUBLE PRECISION NOT NULL DEFAULT 1;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS cost DOUBLE PRECISION NOT NULL DEFAULT 0;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS sell_price DOUBLE PRECISION NOT NULL DEFAULT 0;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS order_status TEXT NOT NULL DEFAULT 'Needed';
ALTER TABLE parts ADD COLUMN IF NOT EXISTS notes TEXT;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS created_at TEXT;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS updated_at TEXT;

CREATE INDEX IF NOT EXISTS idx_suppliers_shop_name ON suppliers (shop_id, name);
CREATE INDEX IF NOT EXISTS idx_suppliers_shop_preferred ON suppliers (shop_id, is_preferred, name);
CREATE INDEX IF NOT EXISTS idx_parts_shop_status ON parts (shop_id, order_status);
CREATE INDEX IF NOT EXISTS idx_parts_shop_supplier ON parts (shop_id, supplier_id);
...
def apply_parts_center_schema(_args: argparse.Namespace) -> None:
    ...
    try:
        with pg_conn:
            with pg_conn.cursor() as cur:
                cur.execute(PARTS_CENTER_SCHEMA_SQL)
        print("parts center schema applied successfully for PostgreSQL database")
