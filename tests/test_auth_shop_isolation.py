...
class AuthShopIsolationTests(unittest.TestCase):
    ...
    def test_finding_detail_customer_review_controls_only_after_prepared_estimate(self):
        ...
        self.assertIn("Secure customer link", after.text)
        self.assertIn("tm-customer-review-link__lock", after.text)
        self.assertIn("tm-customer-review-link__preview", after.text)
        self.assertIn("&bull;&bull;&bull;&bull;", after.text)
        self.assertIn("View Customer Review", after.text)
        self.assertIn("Suggested Customer Message", after.text)
        self.assertIn("Hi Controls, your estimate from Alpha Shop is ready.", after.text)
        self.assertIn("approve, decline, or decide later:", after.text)
        self.assertIn("Please contact us if you have any questions. Thank you!", after.text)
        self.assertIn("Copy Message + Link", after.text)
        self.assertIn("Copy Link Only", after.text)
        self.assertIn("/customer/estimate/", after.text)
        self.assertIn("Secure customer review link ready to share.", after.text)
        self.assertIn("Message copied.", after.text)
        self.assertIn("Link copied.", after.text)
        self.assertIn("Unable to copy. Please try again.", after.text)
        self.assertIn("navigator.clipboard?.writeText", after.text)
        self.assertIn("document.execCommand(\"copy\")", after.text)
        self.assertIn("<button class=\"tm-btn tm-btn-secondary\" type=\"button\" data-copy-customer-review data-copy-kind=\"message\"", after.text)
        self.assertIn("<button class=\"tm-btn tm-btn-secondary\" type=\"button\" data-copy-customer-review data-copy-kind=\"link\"", after.text)
        self.assertNotIn('button.textContent = "Copied";', after.text)
        ...
        preview_html = after.text.split('aria-label="Masked secure customer review link"', 1)[1].split("</span>", 1)[0]
        self.assertNotIn(review_link, preview_html)
        self.assertNotIn(review_token, preview_html)
        visible_text = html.unescape(re.sub(r"<[^>]+>", " ", after.text))
        self.assertIn(review_link, visible_text)
        self.assertIn(chr(8226) * 4, visible_text)
    ...
    def test_customer_estimate_review_valid_token_is_public_and_customer_safe(self):
        ...
        self.assertIn("background:#050e1b;", response.text)
        self.assertIn("background:var(--tm-panel)", response.text)
        self.assertIn("color:#f8fafc !important;", response.text)
        ...
        self.assertNotIn(">Sign Up<", response.text)
        ...
        decision_section = response.text[
            response.text.index('id="customer-decision"'):
            response.text.index('aria-label="Secure review notice"')
        ]
        self.assertIn("Customer Decision", response.text)
        self.assertIn("Review the prepared estimate below, then approve, decline, or decide later.", response.text)
        self.assertIn("Select what should happen with this prepared estimate. Your choice will be sent securely to the shop.", response.text)
        self.assertIn("Approve $812.25", decision_section)
        self.assertIn("Decline", decision_section)
        self.assertIn("Decide Later", decision_section)
        self.assertNotIn("This page is for review only", response.text)
        self.assertNotIn("Confirm Approval", decision_section)
        self.assertNotIn("Confirm Decline", decision_section)
        self.assertNotIn("Confirm Decide Later", decision_section)
        self.assertNotIn("Confirm approval", decision_section)
        self.assertNotIn("Confirm decline", decision_section)
        self.assertNotIn("Confirm decide later", decision_section)
        self.assertNotIn("displayed estimate total of $812.25", decision_section)
        self.assertNotIn(">Cancel<", decision_section)
        self.assertNotIn("tm-public-confirm", decision_section)
        self.assertIn('name="decision" value="approved"', decision_section)
        self.assertIn('name="decision" value="declined"', decision_section)
        self.assertIn('name="decision" value="deferred"', decision_section)
        self.assertNotRegex(decision_section, r'name="(?:shop_id|customer_id|vehicle_id|finding_id|estimate_id|status|estimate_total|total)"')
        self.assertIn('id="customerDecisionStatus" aria-live="polite" aria-atomic="true"', response.text)
        self.assertIn("let isSubmitting = false;", response.text)
        self.assertIn("if (isSubmitting) {", response.text)
        self.assertIn("button.disabled = true;", response.text)
        self.assertIn('button.setAttribute("aria-disabled", "true");', response.text)
        self.assertIn('data-loading-label="Approving..."', response.text)
        self.assertIn('data-loading-label="Declining..."', response.text)
        self.assertIn('data-loading-label="Saving..."', response.text)
        ...
        self.assertNotIn("parts_unit_cost", response.text)
        self.assertNotIn("partsUnitCost", response.text)
    ...
    def test_pro_header_notification_dismiss_deletes_only_shop_notification(self):
        ...
        self.assertIn('method="post" action="/pro/notifications/', dashboard.text)
        self.assertIn('name="csrf_token"', dashboard.text)
        ...
        self.assertIn(".tm-notificationItem{position:relative;display:grid;gap:7px;width:100%;box-sizing:border-box;", notification_css)
        self.assertIn(".tm-notificationItem__trashAction{width:30px;height:30px;", notification_css)
        self.assertNotIn("tm-notificationItem__swipeDismiss", notification_css)
        self.assertNotIn("is-dragging", notification_css)
        self.assertNotIn("is-swiped", notification_css)
        self.assertNotIn("translateX", notification_css)
