...
class AuthShopIsolationTests(unittest.TestCase):
    ...
    def test_finding_detail_customer_review_controls_only_after_prepared_estimate(self):
        ...
        self.assertIn("Unable to copy the link. Please try again.", after.text)
        self.assertIn("<button class=\"tm-btn tm-btn-secondary\" type=\"button\" data-copy-customer-review-link", after.text)
        ...
        self.assertIn("http://localhost/customer/estimate/", visible_text)
    ...
    def test_pro_header_notification_dismiss_deletes_only_shop_notification(self):
        ...
        self.assertIn("const notificationSwipeThreshold = 46", nav_js)
        self.assertIn("const notificationSwipeDirectionThreshold = 8", nav_js)
        self.assertIn("const notificationSwipeHorizontalRatio = 1.35", nav_js)
        self.assertIn("function notificationSwipeDirection(absX, absY)", nav_js)
        self.assertIn("return \"vertical\";", nav_js)
        self.assertIn("e.preventDefault();", nav_js)
        self.assertIn("Math.min(0, Math.max(-notificationSwipeReveal, startOffset + deltaX))", nav_js)
        self.assertIn("if (openNotificationSwipe && openNotificationSwipe !== item) closeNotificationSwipe(openNotificationSwipe);", nav_js)
        self.assertIn("notificationPanel.addEventListener(\"scroll\", closeOpenNotificationSwipe", nav_js)
        self.assertIn("window.addEventListener(\"scroll\", closeOpenNotificationSwipe", nav_js)
        ...
        self.assertIn(".tm-notificationItemShell.is-dragging .tm-notificationItem{transition:none;}", nav_css)
