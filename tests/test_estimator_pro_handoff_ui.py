...
class EstimatorProHandoffUiTests(unittest.TestCase):
    ...
    def test_stage_one_visual_hooks_are_present(self):
        ...
        self.assertIn("Suggested Customer Message", finding_html)
        self.assertIn("Hi {{ customer_first_name }}, your estimate from {{ display_shop_name }} is ready.", finding_html)
        self.assertIn("approve, decline, or decide later:", finding_html)
        self.assertIn("Please contact us if you have any questions. Thank you!", finding_html)
        self.assertIn("data-customer-review-message", finding_html)
        self.assertIn("View Customer Review", finding_html)
        self.assertIn("Copy Message + Link", finding_html)
        self.assertIn("Copy Link Only", finding_html)
        self.assertIn("data-copy-customer-review", finding_html)
        self.assertIn('data-copy-kind="message"', finding_html)
        self.assertIn('data-copy-kind="link"', finding_html)
        self.assertIn("data-copy-customer-review-status", finding_html)
        self.assertIn("navigator.clipboard?.writeText", finding_html)
        self.assertIn('document.execCommand("copy")', finding_html)
        self.assertIn("Message copied.", finding_html)
        self.assertIn("Link copied.", finding_html)
        self.assertIn("Unable to copy. Please try again.", finding_html)
    ...
    def test_stage_two_public_review_template_customer_safe_ui_hooks(self):
        ...
        self.assertIn("Customer Decision", public_review_html)
        self.assertIn("Approve {{ estimate.estimate_total|pro_currency }}", public_review_html)
        self.assertIn("Decline", public_review_html)
        self.assertIn("Decide Later", public_review_html)
        self.assertIn("Review the prepared estimate below, then approve, decline, or decide later.", public_review_html)
        self.assertIn("Select what should happen with this prepared estimate. Your choice will be sent securely to the shop.", public_review_html)
        self.assertNotIn("This page is for review only", public_review_html)
        self.assertNotIn("Confirm Approval", public_review_html)
        self.assertNotIn("Confirm Decline", public_review_html)
        self.assertNotIn("Confirm Decide Later", public_review_html)
        self.assertNotIn("Confirm approval", public_review_html)
        self.assertNotIn("Confirm decline", public_review_html)
        self.assertNotIn("Confirm decide later", public_review_html)
        self.assertNotIn(">Cancel<", public_review_html)
        self.assertNotIn("tm-public-confirm", public_review_html)
        self.assertIn('method="post" action="{{ decision_url }}"', public_review_html)
        self.assertIn('name="decision" value="approved"', public_review_html)
        self.assertIn('name="decision" value="declined"', public_review_html)
        self.assertIn('name="decision" value="deferred"', public_review_html)
        self.assertIn('id="customerDecisionStatus" aria-live="polite" aria-atomic="true"', public_review_html)
        self.assertIn("let isSubmitting = false;", public_review_html)
        self.assertIn("if (isSubmitting) {", public_review_html)
        self.assertIn("button.disabled = true;", public_review_html)
        self.assertIn('button.setAttribute("aria-disabled", "true");', public_review_html)
        self.assertIn('selectedButton.dataset.loadingLabel || "Saving..."', public_review_html)
        self.assertIn('data-loading-label="Approving..."', public_review_html)
        self.assertIn('data-loading-label="Declining..."', public_review_html)
        self.assertIn('data-loading-label="Saving..."', public_review_html)
        self.assertIn("restoreDecisionButtons();", public_review_html)
        self.assertIn("Your approval was received.", public_review_html)
        self.assertIn("Your response was received.", public_review_html)
        self.assertIn("Current recorded decision:", public_review_html)
        self.assertIn("Deciding Later", public_review_html)
        self.assertIn("The repair shop may now contact you, schedule the repair, or begin the approved work according to the shop's workflow.", public_review_html)
        self.assertIn("You declined the prepared service.", public_review_html)
        self.assertIn("may return using this secure estimate link while it remains valid.", public_review_html)
        self.assertIn('role="status" aria-live="polite" aria-atomic="true"', public_review_html)
        self.assertIn('aria-label="Current recorded decision"', public_review_html)
