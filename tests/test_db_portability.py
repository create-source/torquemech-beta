...
class DatabasePortabilityTests(unittest.TestCase):
    ...
    def test_staff_notifications_schema_is_runtime_allowed_for_postgres(self):
        conn = object.__new__(PostgresCompatConnection)
        ...
        self.assertIn("CREATE TABLE IF NOT EXISTS staff_notifications", sql)
        self.assertIn("GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY", sql)
        self.assertNotIn("AUTOINCREMENT", sql)
        self.assertEqual(params, ())

    def test_customer_decision_notification_uses_postgres_compatible_dedupe_insert(self):
        conn = object.__new__(PostgresCompatConnection)
        statements = []

        class FakeCompatConnection:
            def execute(self, sql, params=()):
                translated_sql, translated_params = conn._translate_sql(sql, tuple(params or ()))
                statements.append((translated_sql, translated_params))

            def commit(self):
                statements.append(("COMMIT", ()))

        pro_module.create_customer_decision_notification_if_needed(
            FakeCompatConnection(),
            shop_id=7,
            customer={"id": 11, "first_name": "Ada", "last_name": "Lovelace"},
            vehicle={"id": 13, "year": 2019, "make": "Honda", "model": "Civic"},
            finding={"id": 17, "recommendation": "Alternator Replacement"},
            estimate={"id": 19, "estimate_total": 324.0},
            decision_status="Approved",
            created_at="2026-07-30T12:00:00",
        )

        insert_sql = next(sql for sql, _params in statements if "INSERT INTO staff_notifications" in sql)
        _sql, insert_params = next(item for item in statements if "INSERT INTO staff_notifications" in item[0])
        self.assertNotIn("INSERT OR IGNORE", insert_sql)
        self.assertIn("ON CONFLICT(source_key) DO NOTHING", insert_sql)
        self.assertIn("%s", insert_sql)
        self.assertIn("customer_secure_link:7:11:13:17:19:Approved", insert_params)
