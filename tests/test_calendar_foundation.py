...
class CalendarFoundationTests(unittest.TestCase):
    ...
    def test_public_booking_page_shows_saved_schedule_and_success_copy(self):
        ...
        self.assertNotIn("Back to CRM", response.text)
    ...
    def test_calendar_request_status_actions_and_friendly_notices(self):
        ...
        self.assertIn('href="/pro/dashboard">Back to CRM</a>', pending_page.text)
        self.assertIn('href="/pro/follow-ups">Follow-Ups</a>', pending_page.text)
        self.assertNotIn("Shop Schedule", pending_page.text)
        self.assertEqual(pending_page.text.count('aria-label="Add Appointment"'), 1)
        self.assertEqual(pending_page.text.count("<summary>Add Appointment</summary>"), 1)
        self.assertNotIn('aria-label="Add Appointment" open', pending_page.text)
        self.assertLess(pending_page.text.index("Back to CRM"), pending_page.text.index("Follow-Ups"))
        self.assertLess(pending_page.text.index("Follow-Ups"), pending_page.text.index("Add Appointment"))
        self.assertLess(pending_page.text.index("Add Appointment"), pending_page.text.index("Pending Requests (6)"))
        self.assertLess(pending_page.text.index("Pending Requests (6)"), pending_page.text.index("Confirmed Appointments"))
